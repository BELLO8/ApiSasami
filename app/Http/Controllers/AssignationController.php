...
class AssignationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(empty($vulnerable)){
            return response()->json(array('Message'=>"Vous n\'êtes pas une personne vulnerable"));
        }
        ...
        $validate =  Validator::make($req, [
            ...
            'exists' => 'Ce dispositif n\'existe pas',

        ]);
        $errors = $validate->errors();
        if ($validate->fails()) {
            return response()->json(array('status' => "errors",'message' => $errors->first('reference')),400);
        }
